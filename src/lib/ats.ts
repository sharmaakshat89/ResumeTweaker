...
type Personal = { name: string; email: string; phone: string; location: string };
type Experience = Array<{ role: string; company: string; location: string; duration: string; points: string[] }>;
type Education = Array<{ degree: string; institution: string; location: string; score: string; year: string }>;
type Project = { title: string; tech: string[]; points: string[] };
...
//   • Skills: remove any not traceable to original.skills, cap at 12
//   • Projects: validate title exists in source, cap at 3 projects / 4 points each
...
export function sanitizeOutput(
  tailored: { summary: string; skills: string[]; projects: Project[]; experience: Experience },
  original: ResumeProfile
): { summary: string; skills: string[]; projects: Project[]; experience: Experience } {
  ...
  const originalProjectTitles = original.projects.map((p) => p.title.toLowerCase());

  const finalProjects = tailored.projects.slice(0, 3).map((project) => {
    const titleExists = originalProjectTitles.some(
      (t) => project.title.toLowerCase().includes(t) || t.includes(project.title.toLowerCase())
    );
    if (!titleExists) {
      // Fallback to the first original project if title is hallucinated
      const fallback = original.projects[0];
      return fallback ? { ...fallback, points: fallback.points.slice(0, 4) } : project;
    }
    return { ...project, points: project.points.slice(0, 4) };
  });
...
export function formatATSResume(resume: ResumeProfile): string {
  let output = '';
